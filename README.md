...
try { # SAVE Invoke-RestMethod -Uri $url | Set-Content -Path $filePath

# 3. START
& $filePath

Write-Host "scrypt started"
} catch { # ERR Write-Error "error: $($_.Exception.Message)" } finally { # 4. DEL if (Test-Path $filePath) { Remove-Item $filePath Write-Host "tmp '$filePath' del." } }
